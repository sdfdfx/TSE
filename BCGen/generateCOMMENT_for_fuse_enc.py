...
import numpy as np
from config import *
from fuse_enc_model import *
# from fuse_emb_model import *
...
def _eval_epoch(sess, epoch, mode):
    ...
    with open('./output/refs_10w_cfgsbt_code2.txt', 'w') as f1, open('./output/hyps_10w_cfgsbt_code2.txt', 'w') as f2,\
            open('./output/srcs_10w_cfgsbt_code2.txt', 'w') as f3, open('./output/cfgs_10w_cfgsbt_code2.txt', 'w') as f4:
        while True:
...
with tf.Session() as sess:  # config=config
    ...
    if run_mode == 'train_and_evaluate':
        ...
        if tf.train.latest_checkpoint(model_dir) is not None:
            ...
            logger.info('Restore latest checkpoint in %s' % tf.train.latest_checkpoint(model_dir))
            saver.restore(sess, tf.train.latest_checkpoint(model_dir))
            #saver.restore(sess, "./models_10W_cfgsbt_data/model_148000.ckpt")
