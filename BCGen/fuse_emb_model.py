...
from config import *
from preprocess2 import file_based_input_fn_builder
import os
import csv
import collections
...
with tf.variable_scope('bert'):
    ...
    word_embeds1 = embedder(src_input_ids)
    word_embeds2 = embedder(cfg_input_ids)

    # Creates segment embeddings for each type of tokens.
    segment_embedder = tx.modules.WordEmbedder(
        vocab_size=bert_config.type_vocab_size,
        hparams=bert_config.segment_embed)
    segment_embeds1 = segment_embedder(src_segment_ids)
    segment_embeds2 = segment_embedder(cfg_segment_ids)
    
    # fuse_embedding
    word_embeds = tf.concat([word_embeds1, word_embeds2], 1)
    segment_embeds = tf.concat([segment_embeds1,segment_embeds2], 1)
    ...
    encoder_output = encoder(input_embeds, src_input_length+cfg_input_length)
...
outputs = decoder(
    ...
    memory_sequence_length=src_input_length+cfg_input_length,
