...
from models.fuse_embedding import MainModel
import config
import logging
...
def train(vcblry_size, train_loader, valid_loader, nl_vcblry, dataset_size, is_main_model, logger,
    ...
    print(layer_wise_parameters(model))
...
def modeltest(model_state_dict, vcblry_size, test_loader, nl_vcblry, dataset_size, is_main_model, logger):
    ...
    with open(config.dataset_base_path + '/preds_code_cfgsbt_sbt_emb2.txt', 'w', encoding='utf-8') as fp, \
            open(config.dataset_base_path + '/ref_code_cfgsbt_sbt_emb2.txt', 'w', encoding='utf-8') as fr:
...
if __name__ == '__main__':
    ...
    modelpath = next(os.walk(config.model_base_path+'main/'))[2][-1]
    main_model_state2 = torch.load(f'{config.model_base_path}main/202210271915_best_model_epoch4.pt', map_location='cpu')
    start_time = datetime.now()
    modeltest(main_model_state2, (keycode_vcblry_size, nl_vcblry_size, sbt_vcblry_size, cfg_vcblry_size),
    ...
    ref = []
    hyp = []
    with open(config.dataset_base_path + '/preds_code_cfgsbt_sbt_emb2.txt', 'r', encoding='utf-8') as f1, \
        open(config.dataset_base_path + '/ref_code_cfgsbt_sbt_emb2.txt', 'r', encoding='utf-8') as f2:
        lines1 = f1.readlines()
        lines2 = f2.readlines()
