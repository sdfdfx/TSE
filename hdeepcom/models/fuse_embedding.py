...
class Encoder(nn.Module):
    def __init__(self, src_vcblry_size, cfg_vcblry_size=None):
        super(Encoder, self).__init__()
        self.hdn_size = config.encoder_hdn_size
        self.embedding_dim = config.embedding_dim
        self.embedding = nn.Embedding(src_vcblry_size, self.embedding_dim)
        self.cfg_vcblry_size = cfg_vcblry_size
        if self.cfg_vcblry_size!=None:
            self.embedding2 = nn.Embedding(cfg_vcblry_size, self.embedding_dim)
    ...
    def init_paras(self):
        ...
        if self.cfg_vcblry_size != None:
            self.embedding2.weight.data.normal_(std=1e-4)
    ...
    def forward(self, src, src_length_lst, cfg=None, cfg_length_lst=None):
        src_emb = self.embedding(src)
        if cfg!=None and cfg_length_lst!=None:
            # (len, bs, dim)
            cfg_emb = self.embedding2(cfg)
            fuse_emb = torch.cat([src_emb, cfg_emb], dim=0)
            # fuse_emb = torch.cat([src_emb, cfg_emb], dim=0).transpose(0,2)
            # linear = nn.Linear(src_emb.size()[0]+cfg_emb.size()[0], cfg_emb.size()[0]).to(config.device)
            # fusenn = F.relu(linear(fuse_emb))
            # fusenn = fusenn.transpose(0,2)
            output, hidden = self.gru(fuse_emb)
            res = output[:, :, :self.hdn_size] + output[:, :, self.hdn_size:]
        else:
...
class MainModel(nn.Module):
    def __init__(self, keycode_vcblry_size, nl_vcblry_size, sbt_vcblry_size, cfg_vcblry_size, load_path=None,
                 load_dict=None, is_eval=False):
        super(MainModel, self).__init__()
        self.nl_vcblry_size = nl_vcblry_size
        self.is_eval = is_eval

        self.keycode_encoder = Encoder(keycode_vcblry_size, cfg_vcblry_size)
        self.sbt_encoder = Encoder(sbt_vcblry_size, cfg_vcblry_size)
    ...
    def forward(self, batch_data, nl_bos_idx, is_test=False):
        ...
        nl_batch_data, nl_seq_lens, cfg_batch_data, cfg_seq_lens = batch_data
        keycode_enc_opt, keycode_enc_hdn = self.keycode_encoder(keycode_batch_data, keycode_seq_lens, cfg_batch_data, cfg_seq_lens)
        sbt_enc_opt, sbt_enc_hdn = self.sbt_encoder(sbt_batch_data, sbt_seq_lens, cfg_batch_data, cfg_seq_lens)
