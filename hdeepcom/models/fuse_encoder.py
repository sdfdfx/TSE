...
class Decoder(nn.Module):
    def __init__(self, tgt_vcblry_size, is_pretrain=True):
        ...
        if not self.is_pretrain:
            ...
            self.cfg_attention = AttentionLayer()
    ...
    def forward(self, inputs, last_hidden, keycode_enc_outputs, sbt_enc_outputs=None, cfg_enc_outputs=None):
        ...
        if self.is_pretrain:
            ...
            cfg_atte_weights = self.cfg_attention(last_hidden, cfg_enc_outputs)
            cfg_context = cfg_atte_weights.bmm(cfg_enc_outputs.transpose(0, 1)).transpose(0, 1)
            context = keycode_context + sbt_context + cfg_context
...
class MainModel(nn.Module):
    def __init__(self, keycode_vcblry_size, nl_vcblry_size, sbt_vcblry_size, cfg_vcblry_size, load_path=None,
        ...
        self.cfg_encoder = Encoder(cfg_vcblry_size)
        self.hidden_merge_layer = nn.Linear(3 * config.encoder_hdn_size, config.encoder_hdn_size)
        ...
        if config.use_cuda:
            ...
            self.cfg_encoder = self.cfg_encoder.cuda(config.cuda_id)
        ...
        if load_path or load_dict:
            ...
            self.cfg_encoder.load_state_dict(state_dict['cfg_encoder'])
    ...
    def forward(self, batch_data, nl_bos_idx, is_test=False):
        ...
        cfg_enc_opt, cfg_enc_hdn = self.cfg_encoder(cfg_batch_data, cfg_seq_lens)

        # last_dec_hdn = self.hidden_merge_layer(torch.cat([keycode_enc_hdn[:1], sbt_enc_hdn[:1]], dim=2))
        last_dec_hdn = self.hidden_merge_layer(torch.cat([keycode_enc_hdn[:1], sbt_enc_hdn[:1], cfg_enc_hdn[:1]], dim=2))
        last_dec_hdn = F.relu(last_dec_hdn)

        if is_test:
            return keycode_enc_opt, sbt_enc_opt, cfg_enc_opt, last_dec_hdn
        ...
        for cur_step in range(max_dec_step):
            cur_dec_output, last_dec_hdn = self.decoder(dec_input, last_dec_hdn, keycode_enc_opt, sbt_enc_opt, cfg_enc_opt)
