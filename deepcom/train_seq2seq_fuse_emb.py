...
from models.seq2seq_fuse_embdeding import MainModel
...
def train(vcblry_size, train_loader, valid_loader, nl_vcblry, dataset_size, is_main_model, logger,
    ...
    if not is_main_model:
        model = PreTrainModel(vcblry_size[0], vcblry_size[1], vcblry_size[2])
...
def valid(vcblry_size, valid_loader, cur_state_dict, nl_vcblry, is_main_model, logger):
    if not is_main_model:
        valid_model = PreTrainModel(vcblry_size[0], vcblry_size[1], vcblry_size[2], load_dict=cur_state_dict, is_eval=True)
    else:
...
def modeltest(model_state_dict, vcblry_size, test_loader, nl_vcblry, dataset_size, is_main_model, logger):
    ...
    with open(config.dataset_base_path + '/preds_seq2seq_fuse_emb2.txt', 'w', encoding='utf-8') as fp, \
            open(config.dataset_base_path + '/ref_seq2seq_fuse_emb2.txt', 'w', encoding='utf-8') as fr:
...
def my_collate_fn(batch, keycode_vcblry, nl_vcblry, cfg_vcblry, is_raw_nl=False):
    keycode_batch = []
    cfg_batch = []
    nl_batch = []
    ...
    return keycode_batch, keycode_seq_lens, nl_batch, nl_seq_lens, cfg_batch, cfg_seq_lens
...
class DatasetObject(Dataset):
    def __init__(self, keycode_file_path, nl_file_path, cfg_file_path):
    ...
    def __getitem__(self, idx):
        return self.keycode_set[idx], self.nl_set[idx], self.cfg_set[idx]
...
if __name__ == '__main__':
    ...
    trainset = DatasetObject(config.keycode_trainset_path, config.nl_trainset_path, config.cfg_trainset_path)
    train_loader = DataLoader(dataset=trainset, batch_size=config.batch_size, shuffle=True,
                              collate_fn=lambda *x: my_collate_fn(x, keycode_vcblry, nl_vcblry, cfg_vcblry))
    logger.info('Done')


    logger.info('Init validset and testset...')
    validset = DatasetObject(config.keycode_validset_path, config.nl_validset_path, config.cfg_validset_path)
    ...
    testset = DatasetObject(config.keycode_testset_path, config.nl_testset_path, config.cfg_testset_path)
    ...
    test_loader = DataLoader(dataset=testset, batch_size=config.batch_size, shuffle=False,
                             collate_fn=lambda *x: my_collate_fn(x, keycode_vcblry, nl_vcblry, cfg_vcblry, is_raw_nl=True))
    logger.info('Done.')
    ...
    #
    # # 不加载预训练模型的
    # main_model_state = train((keycode_vcblry_size, nl_vcblry_size, cfg_vcblry_size),
    ...
    main_model_state = torch.load(f'{config.model_base_path}main/202301121250_best_model_epoch3.pt', map_location='cpu')
    start_time = datetime.now()
    modeltest(main_model_state, (keycode_vcblry_size, nl_vcblry_size, cfg_vcblry_size),
    ...
    ref = []
    hyp = []
    with open(config.dataset_base_path + '/preds_seq2seq_fuse_emb2.txt', 'r', encoding='utf-8') as f1, \
        open(config.dataset_base_path + '/ref_seq2seq_fuse_emb2.txt', 'r', encoding='utf-8') as f2:
        lines1 = f1.readlines()
        lines2 = f2.readlines()
