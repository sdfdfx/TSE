...
from models.seq2seq import MainModel
...
def train(vcblry_size, train_loader, valid_loader, nl_vcblry, dataset_size, is_main_model, logger,
    ...
    if not is_main_model:
        ...
        model = MainModel(vcblry_size[0], vcblry_size[1])
        # model.keycode_encoder.load_state_dict(pretrain_kyc_enc, strict=False)
        paras = list(model.keycode_encoder.parameters()) + list(model.hidden_merge_layer.parameters()) + \
...
def valid(vcblry_size, valid_loader, cur_state_dict, nl_vcblry, is_main_model, logger):
    if not is_main_model:
        ...
        valid_model = MainModel(vcblry_size[0], vcblry_size[1], load_dict=cur_state_dict, is_eval=True)
...
################################################################################################################
def modeltest(model_state_dict, vcblry_size, test_loader, nl_vcblry, keycode_vcblry, dataset_size, is_main_model, logger):
    test_model = MainModel(vcblry_size[0], vcblry_size[1], load_dict=model_state_dict, is_eval=True)
    ...
    for b_idx, batch in tqdm(enumerate(test_loader)):
        ...
        with torch.no_grad():
            if b_idx == 5767:
                ...
                batch_idx_seqs, batch_idx_attens = beam_search_sents(test_model, nl_vcblry, batch_size, keycode_enc_opt, dec_hidden)

                candidates = trans_idx2word(batch_idx_seqs, nl_vcblry)
                batch_s_bleu = cal_batch_sentence_bleu(nl_batch, candidates)

                inputseq = trans_idx2word(batch[0].cpu().numpy(), keycode_vcblry)
                col = []
                for i in inputseq:
                    if len(i) !=0:
                        col.append(i[0])
                ...
                datamap = np.array(batch_idx_attens[0])[2:-2,1:-1]
                sns.heatmap(pd.DataFrame(datamap, index=candidates[0][1:-1], columns=col[1:-1]), xticklabels=1, yticklabels=1)
    ...
    with open(config.dataset_base_path + '/preds_seq2seq_sbtdeepcom2.txt', 'w', encoding='utf-8') as fp, \
            open(config.dataset_base_path + '/ref_seq2seq_sbtdeepcom2.txt', 'w', encoding='utf-8') as fr:
    ...
    for word_seq in batch:
        ...
        for word in word_seq:
            if word not in vcblry.word2idx:
                ...
                if word.split('_')[0] not in vcblry.word2idx:
                    cur_idx_seq.append(vcblry.word2idx['<UNK>'])
                else:
                    cur_idx_seq.append(vcblry.word2idx[word.split('_')[0]])
            else:
...
def my_collate_fn(batch, keycode_vcblry, nl_vcblry, is_raw_nl=False):
    keycode_batch = []
    nl_batch = []
...
if __name__ == '__main__':
    ...
    trainset = DatasetObject(config.keycode_trainset_path, config.nl_trainset_path)
    train_loader = DataLoader(dataset=trainset, batch_size=config.batch_size, shuffle=True,
                              collate_fn=lambda *x: my_collate_fn(x, keycode_vcblry, nl_vcblry))
    logger.info('Done')

    logger.info('Init validset and testset...')
    validset = DatasetObject(config.keycode_validset_path, config.nl_validset_path)
    ...
    testset = DatasetObject(config.keycode_testset_path, config.nl_testset_path)
    ...
    test_loader = DataLoader(dataset=testset, batch_size=config.batch_size, shuffle=False,
                             collate_fn=lambda *x: my_collate_fn(x, keycode_vcblry, nl_vcblry, is_raw_nl=True))
    logger.info('Done.')
    ...
    #
    # # 不加载预训练模型的
    # main_model_state = train((keycode_vcblry_size, nl_vcblry_size),
    ...
    # sbt
    main_model_state = torch.load(f'{config.model_base_path}main/202301082120_best_model_epoch5.pt', map_location='cpu')
    
    start_time = datetime.now()
    modeltest(main_model_state, (keycode_vcblry_size, nl_vcblry_size),
         test_loader, nl_vcblry, keycode_vcblry, len(testset), is_main_model=True, logger=logger)
    logger.info('Testing done. [Time taken: {!s}]'.format(datetime.now() - start_time))

    ref = []
    hyp = []
    with open(config.dataset_base_path + '/preds_seq2seq_sbtdeepcom2.txt', 'r', encoding='utf-8') as f1, \
        open(config.dataset_base_path + '/ref_seq2seq_sbtdeepcom2.txt', 'r', encoding='utf-8') as f2:
        lines1 = f1.readlines()
        lines2 = f2.readlines()
