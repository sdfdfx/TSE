...
class Encoder(nn.Module):
    ...
    def forward(self, src, src_length_lst, cfg=None, cfg_length_lst=None):
        ...
        if cfg!=None and cfg_length_lst!=None:
            ...
            # linear = nn.Linear(src_emb.size()[0]+cfg_emb.size()[0], src_emb.size()[0]).to(config.device)
...
class MainModel(nn.Module):
    def __init__(self, keycode_vcblry_size, nl_vcblry_size, cfg_vcblry_size, load_path=None, load_dict=None, is_eval=False):
    ...
    def forward(self, batch_data, nl_bos_idx, is_test=False):
        keycode_batch_data, keycode_seq_lens, nl_batch_data, nl_seq_lens, cfg_batch_data, cfg_seq_lens = batch_data
